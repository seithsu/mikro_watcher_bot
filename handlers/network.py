...
from .utils import (
    ...
    set_cache_with_ts,
    get_cache_if_fresh,
    put_callback_payload,
    get_callback_payload,
    with_menu_timestamp)
from mikrotik import (
    get_interfaces, get_traffic, get_dhcp_leases, get_ip_addresses,
    run_ip_scan, find_free_ips, send_wol
)
    ...
    try:
        ...
        for iface in interfaces:
            ...
            keyboard.append([InlineKeyboardButton(
                btn_text, callback_data=f"ifacedetailk_{tok}")])
...
async def callback_ifacedetail(
        update: Update,
    ...
    if data.startswith('ifacedetailk_'):
        ...
        iface_name = get_callback_payload(
            context.bot_data, "ifacedetail", token, ttl_seconds=1800)
        if not iface_name:
    ...
    try:
        ...
        tok_detail = put_callback_payload(
            context.bot_data, "ifacedetail", iface_name)
        tok_traffic = put_callback_payload(
            context.bot_data, "traffic", iface_name)
        keyboard = [
            [
                InlineKeyboardButton(
                    "🔄 Refresh Detail", callback_data=f"ifacedetailk_{tok_detail}")], [
                InlineKeyboardButton(
                    f"📈 Cek Traffic {iface_name}", callback_data=f"traffick_{tok_traffic}")], [
                        InlineKeyboardButton(
                            "🔙 Kembali ke List Interface", callback_data="cmd_interface")], [
                                InlineKeyboardButton(
                                    "🔙 Menu Utama", callback_data="cmd_start")]]
...
async def cmd_traffic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query and update.callback_query.data.startswith(
        ...
        interface_name = get_callback_payload(
            context.bot_data, "traffic", token, ttl_seconds=1800)
    ...
    if not interface_name:
        ...
        try:
            ...
            for iface in interfaces:
                ...
                keyboard.append([InlineKeyboardButton(
                    f"📊 {name}", callback_data=f"traffick_{tok}")])
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            pesan_traffic = with_menu_timestamp(
                "🌐 Pilih interface untuk cek traffic:")
            if update.callback_query:
                try:
    ...
    try:
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "🔄 Refresh Traffic", callback_data=f"traffick_{
                        put_callback_payload(
                            context.bot_data, 'traffic', interface_name)}")], [
                InlineKeyboardButton(
                    "🔙 Kembali ke List Interface", callback_data="cmd_traffic")], [
                                InlineKeyboardButton(
                                    "🔙 Menu Utama", callback_data="cmd_start")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
def _format_scan_page(
    ...
    if not interface_token:
        # Safety fallback untuk legacy flow: paging dinonaktifkan jika token
        # hilang.
        keyboard = [[InlineKeyboardButton(
            "🔄 Rescan", callback_data="cmd_scan")]]
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                "◀️ Prev",
                callback_data=f"scpk_{interface_token}_{
                    page - 1}"))
    ...
    keyboard.append([InlineKeyboardButton(
        "🔄 Rescan", callback_data=f"sck_{interface_token}")])
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        pesan_scan = with_menu_timestamp(
            "🌐 Pilih interface yang ingin di-scan:")
        if update.callback_query:
            try:
...
async def _do_scan(
    ...
    try:
        ...
        if not interface_token:
            interface_token = put_callback_payload(
                context.bot_data, "scan_iface", interface_name)
        scan_key = f"scan_result_tok_{interface_token}"
        set_cache_with_ts(context.bot_data, scan_key, devices)

        text, reply_markup = _format_scan_page(
            devices, interface_name, page=0, interface_token=interface_token)
        await msg_to_edit.edit_text(text, parse_mode='HTML', reply_markup=append_back_button(reply_markup))

        catat(
            user.id,
            user.username,
            f"/scan {interface_name}",
            f"berhasil ({
                len(devices)} devices)")
        ...
        if '10038' in err_str or 'socket' in err_str.lower():
            pesan_err = f"❌ <b>Scan gagal:</b> Interface <b>{
                escape_html(interface_name)}</b> sedang tidak aktif (DOWN) atau terputus."
        else:
...
async def callback_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('scpk_'):
        ...
        try:
            ...
            return
        interface_name = get_callback_payload(
            context.bot_data, "scan_iface", token, ttl_seconds=1800)
        if not interface_name:
            await query.answer("Data scan expired. Silakan rescan.", show_alert=True)
            return
        scan_key = f"scan_result_tok_{token}"
        devices = get_cache_if_fresh(
            context.bot_data, scan_key, ttl_seconds=1800)
        if not devices:
            await query.answer("Data scan expired. Silakan rescan.", show_alert=True)
            return
        text, reply_markup = _format_scan_page(
            devices, interface_name, page, interface_token=token)
        await query.answer()
        try:
...
async def cmd_freeip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for addr in addrs:
            ...
            cidr = addr.get('address', '').split(
                '/')[1] if '/' in addr.get('address', '') else '24'
            if network:
                full_network = f"{network}/{cidr}"
                tok = put_callback_payload(
                    context.bot_data, "freeip_net", full_network)
                keyboard.append([InlineKeyboardButton(
                    f"🔍 {full_network}", callback_data=f"freeipk_{tok}")])
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        pesan_free = with_menu_timestamp(
            "🔍 <b>IP KOSONG (FREE IP)</b>\nPilih sub-jaringan yang ingin diperiksa:")
        if update.callback_query:
            try:
...
async def callback_freeip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('fipagek_'):
        ...
        try:
            ...
            await query.answer("Data tidak valid.")
            return
        network = get_callback_payload(
            context.bot_data, "freeip_net", token, ttl_seconds=1800)
        if not network:
            await query.answer("Data waktu scan sudah kedaluwarsa. Silakan Rescan.", show_alert=True)
            return
        cache_key = f"freeip_res_tok_{token}"
        result = get_cache_if_fresh(
            context.bot_data, cache_key, ttl_seconds=1800)
        if not result:
            await query.answer("Data waktu scan sudah kedaluwarsa. Silakan Rescan.", show_alert=True)
            return
        text, reply_markup = _format_freeip_page(
            network, result, page, network_token=token)
        try:
            await query.edit_message_text(text, parse_mode='HTML', reply_markup=append_back_button(reply_markup))
        except Exception as e:
            logger.debug("Suppressed non-fatal exception: %s", e)
...
def _format_freeip_page(network, result, page=0, limit=10, network_token=None):
    ...
    if free_qty == 0:
        ...
        text += f"🟢 <b><u>Mencetak {limit} IP Kosong (Hal {
            page + 1}/{total_pages}):</u></b>\n"
        for ip in page_ips:
            text += f"<code>{ip}</code>\n"
    ...
    if not network_token:
        keyboard = [[InlineKeyboardButton(
            "🔄 Rescan", callback_data="cmd_freeip")]]
        return text, InlineKeyboardMarkup(keyboard)
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                "◀️ Prev",
                callback_data=f"fipagek_{network_token}_{
                    page - 1}"))
    if page < total_pages - 1:
        nav_buttons.append(
            InlineKeyboardButton(
    ...
    keyboard.append([InlineKeyboardButton(
        "🔄 Rescan", callback_data=f"freeipk_{network_token}")])
...
async def _do_freeip(update: Update, context, network, network_token=None):
    ...
    try:
        ...
        if not network_token:
            network_token = put_callback_payload(
                context.bot_data, "freeip_net", network)
        cache_key = f"freeip_res_tok_{network_token}"
        set_cache_with_ts(context.bot_data, cache_key, result)

        text, reply_markup = _format_freeip_page(
            network, result, page=0, network_token=network_token)
...
def _format_dhcp_page(leases, page, per_page=10):
    ...
    for i, lease in enumerate(page_leases, start=start + 1):
        ...
        status_tag = "📌" if lease.get('dynamic') is False else "🔄"
    ...
    keyboard.append([InlineKeyboardButton(
        "🔄 Refresh", callback_data=f"dhcp_page_{page}")])
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    try:
        ...
        if not leases:
            text = with_menu_timestamp(
                "ℹ️ Tidak ada perangkat tersimpan di DHCP leases.")
            if msg:
        ...
        keyboard = []
        for lease in leases:
            name = lease.get('host', lease.get('comment', lease['address']))
            mac = lease.get('mac')
            ...
            if len(name) > 15:
                name = name[:15] + ".."
            label = f"💻 {name} ({lease['address']})"
            mac_clean = mac.replace(':', '')
            cb_data = f"wol_{mac_clean}"

            context.bot_data[cb_data] = mac
            # W8 FIX: Store timestamp for 5-minute TTL (cek di callback_wol di
            # bot.py)
