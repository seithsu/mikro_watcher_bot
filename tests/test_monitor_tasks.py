...
class TestQueueChangeLogDetection:
    def test_queue_change_log_detected_from_system_info(self):
        from monitor.tasks import _is_queue_change_log

        assert _is_queue_change_log(
            "system,info",
            "simple queue changed by admin: FARMASI IGD",
        ) is True

    def test_non_queue_info_log_not_detected(self):
        from monitor.tasks import _is_queue_change_log
...
class TestMonitorTaskLoops:
    ...
    async def test_task_monitor_logs_forwards_queue_change_info_log(self, monkeypatch):
        ...
        logs_tick = [{
            "time": "10:00:01",
            "topics": "system,info",
            "message": "simple queue changed by admin: FARMASI IGD",
        }]
        calls = {"n": 0}
