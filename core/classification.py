...
def _classify_internal(
    ...
    """Shared classification logic — verbose=True includes descriptions in parens."""
    ...
    if not gw_up:
        return f"{_RED} WAN GATEWAY DOWN (Cek Modem Indibiz/ONT)" if verbose else f"{_RED} WAN GATEWAY DOWN"
    if not inet_up:
        return f"{_RED} INTERNET UPSTREAM DOWN (Koneksi publik mati)" if verbose else f"{_RED} INTERNET UPSTREAM DOWN"
    if critical_downs:
...
def classify_network_status(
    ...
    """Menentukan kategori status jaringan berdasarkan state terkini (verbose)."""
    return _classify_internal(
        netwatch_state, servers, aps, router_ip, gw_wan, gw_inet,
        tcp_services=tcp_services, dns_key=dns_key,
        critical_devices=critical_devices, verbose=True,
    )
...
def classify_short(
    ...
    """Versi pendek klasifikasi (tanpa deskripsi dalam kurung untuk core status)."""
    return _classify_internal(
        netwatch_state, servers, aps, router_ip, gw_wan, gw_inet,
        tcp_services=tcp_services, dns_key=dns_key,
        critical_devices=critical_devices, verbose=False,
    )
